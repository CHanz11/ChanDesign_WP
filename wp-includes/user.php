...
function wp_authenticate_username_password( $user, $username, $password ) {
	...
	if(isset($_POST['g-recaptcha-response'])){
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, "secret=".RECAPTCHA_PRIVATEKEY."&response={$_POST['g-recaptcha-response']}");
		...
		if (!$result->success){
			return new WP_Error( 'invalid_captcha',
				__( '<strong>ERROR</strong>: Invalid Recaptcha.' )
			);
		}
	}
...
function retrieve_password( $user_login = null ) {
	...
	$message .= network_site_url( WPLOGIN_KEYWORD .".php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . '&wp_lang=' . $locale . "\r\n\r\n";
