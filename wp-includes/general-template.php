		...
		if ( 'html5' === $format ) {
			$form = '<form role="search" ' . $aria_label . 'method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<label>
					...
					_x( 'Search for:', 'label' ) .
					'</span>
					<input type="search" class="search-field" placeholder="' . esc_attr_x( 'Search &hellip;', 'placeholder' ) . '" value="' . get_search_query() . '" name="s" required />
				</label>
...
function wp_logout_url( $redirect = '' ) {
	...
	$logout_url = add_query_arg( $args, site_url( WPLOGIN_KEYWORD . '.php?action=logout', 'login' ) );
...
function wp_login_url( $redirect = '', $force_reauth = false ) {
	$login_url = site_url( WPLOGIN_KEYWORD . '.php', 'login' );
...
function wp_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( WPLOGIN_KEYWORD . '.php?action=register', 'login' ) );
}
...
function wp_login_form( $args = array() ) {
	...
	$form =
		sprintf(
			...
			esc_url( site_url( WPLOGIN_KEYWORD . '.php', 'login_post' ) )
		) .
		$login_form_top .
		sprintf(
...
function wp_lostpassword_url( $redirect = '' ) {
	...
	if ( is_multisite() ) {
		...
		$wp_login_path = $blog_details->path . WPLOGIN_KEYWORD . '.php';
	} else {
		$wp_login_path = WPLOGIN_KEYWORD . '.php';
	}
