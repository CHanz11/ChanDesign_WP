...
<div class="wrap">
  ...
  <div class="editwidget" <?php echo $width; ?>>
	...
	?>
      ...
      <p class="describe">
        <?php _e( 'Select both the sidebar for this widget and the position of the widget in that sidebar.' ); ?></p>
	...
	?>
      ...
      <div class="widget-control-actions">
        <div class="alignleft">
          ...
          <input type="submit" name="removewidget" id="removewidget"
            class="button-link button-link-delete widget-control-remove" value="<?php esc_attr_e( 'Delete' ); ?>" />
...
?>
  ...
  <div class="widget-liquid-left">
    ...
    <button style="margin-top:30px;" id="add_all" class="button button-primary" onclick="add_all()">Add All Custom
      Html</button>
    <script>
    function add_all() {


      const customHTML = document.getElementById("widget-6_custom_html-__i__");
      const widgetTop = customHTML.querySelector('.widget-top');
      widgetTop.click();

      const ulList = document.querySelector('.widgets-chooser-sidebars').getElementsByTagName('li');
      for (var i = 0; i < ulList.length; i++) {
        const selectButton = ulList[i].getElementsByTagName('button');
        selectButton[0].click();

        const add = document.querySelector('.widgets-chooser-add');
        add.click();
      ...
      <div id="available-widgets" class="widgets-holder-wrap">
        <div class="sidebar-name">
          ...
          <h2><?php _e( 'Available Widgets' ); ?> <span
              id="removing-widget"><?php _ex( 'Deactivate', 'removing-widget' ); ?> <span></span></span></h2>
        </div>
        <div class="widget-holder">
          <div class="sidebar-description">
            <p class="description">
              <?php _e( 'To activate a widget drag it to a sidebar or click on it. To deactivate a widget and delete its settings, drag it back.' ); ?>
            </p>
          </div>
          <div id="widget-list">
            <?php wp_list_widgets(); ?>
...
foreach ( $wp_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( str_contains( $registered_sidebar['class'], 'inactive-sidebar' ) || str_starts_with( $sidebar, 'orphaned_widgets' ) ) {
      ...
      <div class="<?php echo esc_attr( $wrap_class ); ?>">
        ...
        <?php if ( $is_inactive_widgets ) { ?>
        <p class="description">
          <?php _e( 'This will clear all items from the inactive widgets list. You will not be able to restore any customizations.' ); ?>
        </p>
        <?php } ?>
      </div>
      <?php
