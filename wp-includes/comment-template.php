 ...
 */
function comment_form( $recaptcha_key = '',$recaptcha_privite = '', $args = array(), $post_id = null) {
	if ( null === $post_id )
		$post_id = get_the_ID();
	...
	$fields   =  array(
		'author'  => '<p class="comment-form-author">' . '<label for="author">' . __( 'Name' ) . ( $req ? ' <span class="required" style="color:red;">*</span>' : '' ) . '</label> ' .
					 '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" maxlength="245"' . $html_req . ' /></p>',
		'email'   => '<p class="comment-form-email"><label for="email">' . __( 'Email' ) . ( $req ? ' <span class="required" style="color:red;">*</span>' : '' ) . '</label> ' .
					    '<input id="email" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" maxlength="100" aria-describedby="email-notes"' . $html_req . ' /></p>
							<input hidden checked id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />',
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<script src="https://code.jquery.com/jquery-3.3.1.min.js"></script>
							<script>$(document).ready(function(){ $(".g-recaptcha").css({ "transform":"scale(0.85)", "-webkit-transform":"scale(0.85)", "transform-origin":"0 0", "-webkit-transform-origin":"0 0"})
							$("#submit").click(function(){ if(grecaptcha.getResponse() == "") {var $recaptcha = document.querySelector("#g-recaptcha-response"); $recaptcha.setAttribute("required", "required");	$(".g-recaptcha iframe").css({ "border":"1px solid #f25252", "box-shadow":"2px 0px 10px #ff5454"});	}});});	</script><p class="comment-form-comment"><label for="comment">' . _x( 'Comment <span style="color:red;">*</span>', 'noun') . '</label> <textarea id="comment" name="comment" cols="45" rows="8" maxlength="65525" required="required"></textarea></p>'. '<script src="https://www.google.com/recaptcha/api.js"></script>'.
		...
		'comment_notes_after'  => '',
		'action'               => site_url( '/wp-systcon/wp-comments-post.php' ),
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		...
		'submit_button'        => '<input name="%1$s" type="submit" id="%2$s" class="%3$s" value="%4$s" onclick="rme"/>',
...
<div id="respond" class="comment-respond">
  ...
  <form action="<?php echo esc_url( $args['action'] ); ?>" method="post"
    id="<?php echo esc_attr( $args['id_form'] ); ?>" class="<?php echo esc_attr( $args['class_form'] ); ?>">
    <?php
				/**
